    ...
    for r in [2000, 5000, 15000]:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                return [], f"카카오 API 키 인증 오류 (Status Code: {res.status_code}). REST API 키를 확인하세요."
...
with col_rate:
    ...
    if target_curr == "KRW":
        ...
        st.caption(
            f"스프레드 **{st.session_state['spread_rate']}%** | 우대율 **{st.session_state['discount_rate']}%** 적용 | 실제 마진: 1 {target_curr}당 {fmt.format(actual_spread_amount)}"
        )
