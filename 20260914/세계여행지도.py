...
st.set_page_config(
    page_title="세계 여행 대시보드", layout="wide", page_icon="✈️"
)
    ...
    if folder.exists():
      for f in folder.iterdir():
        if f.is_file() and any(
            f.name.lower().endswith(ext) for ext in font_extensions
        ):
...
# -------------------------------------------------------------
# 1. API 키 로드 (에러 없는 안전한 2단계 감지)
# -------------------------------------------------------------
def get_env_or_secret(key_name):
  # 1. VSCode 로컬 환경 변수 (.env) 최우선
  val = os.getenv(key_name, "").strip()
  if val:
    return val
  # 2. 배포 환경 (Streamlit Secrets) 안전 조회
  try:
...
EXCHANGERATE_API_KEY = (
    get_env_or_secret("EXCHANGERATE_API_KEY")
    or get_env_or_secret("EXCHANGE_API_KEY")
)
OPENWEATHER_API_KEY = (
    get_env_or_secret("OPENWEATHER_API_KEY")
    or get_env_or_secret("OPEN_WEATHER_API_KEY")
    or get_env_or_secret("WEATHER_API_KEY")
)
KAKAO_MAP_API_KEY = (
    get_env_or_secret("KAKAO_MAP_API_KEY")
    or get_env_or_secret("KAKAO_REST_API_KEY")
    or get_env_or_secret("KAKAO_API_KEY")
    or get_env_or_secret("KAKAO_KEY")
)
...
OVERSEAS_PLACES_DB = {
    "모스크바": [
        {
            "name": "화이트 래빗 (White Rabbit)",
            "category": "레스토랑",
            "lat": 55.7482,
            "lng": 37.5835,
            "desc": "글래스 돔에서 시내를 내려다보는 파인다이닝",
        },
        {
            "name": "카페 푸시킨 (Кафе Пушкинъ)",
            "category": "레스토랑",
            "lat": 55.7645,
            "lng": 37.6045,
            "desc": "19세기 귀족 저택 분위기의 비프 스트로가노프",
        },
        {
            "name": "스톨로바야 57 (Столовая 57)",
            "category": "레스토랑",
            "lat": 55.7548,
            "lng": 37.6215,
            "desc": "굼(GUM) 백화점 내 소련식 뷔페 식당",
        },
        {
            "name": "더블비 커피 (Double B)",
            "category": "카페",
            "lat": 55.7505,
            "lng": 37.5925,
            "desc": "러시아 대표 감성 스페셜티 카페",
        },
        {
            "name": "코페마니아 (Coffeomania)",
            "category": "카페",
            "lat": 55.7602,
            "lng": 37.6185,
            "desc": "부드러운 시그니처 라프 커피(Raf)",
        },
    ],
    "상트페테르부르크": [
        {
            "name": "문학 카페 (Литературное кафе)",
            "category": "레스토랑",
            "lat": 59.9362,
            "lng": 30.3185,
            "desc": "푸시킨의 단골 러시아 정통 레스토랑",
        },
        {
            "name": "테레목 (Теремок)",
            "category": "레스토랑",
    ...
    "파리": [
        {
            "name": "르 불롱제 (Le Bouillon Chartier)",
            "category": "레스토랑",
            "lat": 48.8718,
            "lng": 2.3432,
            "desc": "100년 역사의 프랑스 전통 가정식",
        },
        {
...
def search_korea_location(query_text):
  ...
  try:
    ...
    res_kw = requests.get(
        url_kw, headers=headers, params={"query": q, "size": 5}, timeout=4
    )
  ...
  if not results:
    try:
      ...
      res_addr = requests.get(
          url_addr, headers=headers, params={"query": q, "size": 5}, timeout=4
      )
...
travel_mode = st.sidebar.radio(
    "범위를 선택하세요:", ["🇰🇷 국내 여행", "✈️ 해외 여행"], horizontal=True
)
...
if travel_mode == "🇰🇷 국내 여행":
  ...
  if suggestions:
    ...
    chosen_label = st.sidebar.radio(
        "지역 목록:", labels, index=0, label_visibility="collapsed"
    )
    place_info = next(s for s in suggestions if s["label"] == chosen_label)
  else:
    if not KAKAO_MAP_API_KEY:
      st.sidebar.error(
          ".env 파일에 KAKAO_MAP_API_KEY가 올바르게 입력되어 있는지 확인하세요."
      )
    else:
      st.sidebar.warning(
          "검색된 장소가 없어 서울특별시청으로 기본 설정됩니다."
      )
    place_info = {
        "label": "🇰🇷 서울특별시청",
        "display_name": "서울",
  ...
  overseas_query = st.sidebar.text_input(
      "도시명을 입력하세요:", value="모스크바"
  ).strip()
  suggestions = search_overseas_places(overseas_query)
  labels = [s["label"] for s in suggestions]
  chosen_label = st.sidebar.radio(
      "도시 목록:", labels, index=0, label_visibility="collapsed"
  )
  place_info = next(s for s in suggestions if s["label"] == chosen_label)

  final_curr = st.sidebar.text_input(
      "적용 통화:", value=place_info["currency"]
  ).strip().upper()
  place_info["currency"] = final_curr if final_curr else place_info["currency"]


# -------------------------------------------------------------
# 5. 좌표 기반 음식점 3개, 카페 3개 수신
# -------------------------------------------------------------
  ...
  for r in [2000, 5000, 15000]:
    ...
    try:
      ...
      if res.status_code == 200:
        ...
        return (
            [],
            f"카카오 API 키 인증 오류 (Status Code: {res.status_code}). REST API"
            " 키를 확인하세요.",
        )
...
def fetch_exchange_rate(target_currency):
  if target_currency == "KRW":
    return 1.0
  url = (
      f"https://v6.exchangerate-api.com/v6/{EXCHANGERATE_API_KEY}/latest/KRW"
  )
  try:
...
if selected_places:
  st.markdown(
      f"**🌟 {clean_title} 주변 추천 장소 ({len(selected_places)}곳):**"
  )
    ...
    for idx, pl in enumerate(res_list):
      with r_cols[idx]:
        ...
        if pl.get("url"):
          ...
          st.markdown(
              f"[🔗 구글 지도 길찾기](https://www.google.com/maps/search/{pl['name']})"
          )
...
if target_curr == "KRW":
  ...
  actual_spread_amount = basic_spread * (
      1.0 - (st.session_state["discount_rate"] / 100.0)
  )
...
with col_rate:
  ...
  if target_curr == "KRW":
    ...
    st.caption(
        f"스프레드 **{st.session_state['spread_rate']}%** | 우대율"
        f" **{st.session_state['discount_rate']}%** 적용 | 실제 마진: 1"
        f" {target_curr}당 {fmt.format(actual_spread_amount)}"
    )
...
# -------------------------------------------------------------
with st.expander(
    f"환율 계산기 & 은행 우대율 설정 ({target_curr})", expanded=True
):
  ...
  with s_col1:
    new_spread = st.slider(
        ...
        min_value=0.5,
        max_value=4.0,
        value=float(st.session_state["spread_rate"]),
        step=0.05,
        key="slider_spread",
    )
  ...
  with s_col2:
    new_discount = st.slider(
        ...
        min_value=0,
        max_value=100,
        value=int(st.session_state["discount_rate"]),
        step=5,
        key="slider_discount",
    )
  ...
  if is_krw_to_foreign:
    ...
    st.success(
        f"💜 **{amount_input:,.0f} KRW** ➡️ **{converted_result:,.2f}"
        f" {target_curr}** (적용 환율: 1 {target_curr} ="
        f" {fmt.format(cash_buy)})"
    )
  else:
    converted_result = float(amount_input * cash_sell)
    st.success(
        f"💜 **{amount_input:,.2f} {target_curr}** ➡️"
        f" **{converted_result:,.0f} KRW** (적용 환율: 1 {target_curr} ="
        f" {fmt.format(cash_sell)})"
    )
...
with b_col1:
  st.link_button(
      "📍 구글 지도에서 명소/맛집 보기",
      f"https://www.google.com/maps/search/{clean_title}+맛집",
  )
with b_col2:
  st.link_button(
      "🏨 아고다 숙소 최저가 검색",
      f"https://www.agoda.com/search?city={clean_title}",
  )
with b_col3:
  st.link_button(
      "✈️ 트립어드바이저 여행 후기",
      f"https://www.tripadvisor.com/Search?q={clean_title}",
  )